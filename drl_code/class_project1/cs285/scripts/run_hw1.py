...
def run_training_loop(params):
    ...
    print("Max episode length: ", env.spec.max_episode_steps)
    print("EP length: ", params['ep_len'])
    ...
    #--------
    import pandas as pd
    df = pd.DataFrame(columns=['itr', 'eval_mean_ret', 'eval_std_ret', 'train_mean_ret', 'train_std_ret'])
    #--------
    ...
    for itr in range(params['n_iter']):
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
            ...
            indices = np.random.choice(buffer_size, min(buffer_size, params['train_batch_size']), replace=False)
        ...
        if log_metrics:
            ...
            row = [itr, logs['Eval_AverageReturn'], logs['Eval_StdReturn'], logs['Train_AverageReturn'], logs['Train_StdReturn']]
            df.loc[len(df)] = row
